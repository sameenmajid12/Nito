import { Pressable, StyleSheet, View, Alert, ActivityIndicator } from "react-native"; 
...
function AddProfilePicture({image, setImage}) {
    ...
    if (!result.canceled) {
      ...
      try {
        ...
        manipulator.resize({ width: 250, height: 250 });
         manipulator.compress(0.8, ImageManipulator.SaveFormat.JPEG );
         const manipulatedResult = await manipulator.renderAsync();
        const resizedUri = manipulatedResult.uri;
        setImage(resizedUri); 

      } catch (error) {
        console.error("Error manipulating image:", error);
        Alert.alert("Image Error", "Could not process the image. Please try another one.");
        setImage(null);
      }
    } else {
        setImage(null); 
  ...
  return (
    <Pressable onPress={pickProfilePic} style={styles.cameraContainer}>
      {isLoadingImage ? ( 
        <ActivityIndicator size="large" color={colors.primary} /> 
      ) : image ? (
        <>
          <Image source={{ uri: image }} style={styles.profilePicture}></Image>
          <Pressable onPress={handleClearImage} style={styles.closeIconContainer}>
